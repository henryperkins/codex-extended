  ...
  test("should trigger at 90% context usage (10% remaining)", () => {
    ...
    for (let i = 0; i < 3; i++) {
      items.push({
        ...
        content: [
          { type: i % 2 === 0 ? "input_text" : "output_text", text: largeText },
        ],
      });
    }

    // Test with a model that has limited context
    const percentRemaining = calculateContextPercentRemaining(
      items,
      "gpt-3.5-turbo-16k",
    );
    ...
    const percentRemainingHigh = calculateContextPercentRemaining(
      items,
      "gpt-3.5-turbo-16k",
    );
  ...
  test("should show context usage increases with more messages", () => {
    ...
    for (let i = 0; i < 5; i++) {
      items.push({
        ...
        role: i % 2 === 0 ? "user" : "assistant",
        content: [
          { type: i % 2 === 0 ? "input_text" : "output_text", text: message },
        ],
      });
    }

    const percentAfter5 = calculateContextPercentRemaining(
      items,
      "gpt-3.5-turbo-16k",
    );
    ...
    const percentAfter10 = calculateContextPercentRemaining(
      items,
      "gpt-3.5-turbo-16k",
    );
