...
describe("Tool Selection with RAG", () => {
  ...
  test("should limit tools to maxTools parameter", () => {
    const query =
      "search for Python tutorials and then create a new python file";
  ...
  test("shouldIncludeTool should correctly identify relevant tools", () => {
    ...
    expect(shouldIncludeTool("search for React tutorials", "web_search")).toBe(
      true,
    );
    expect(shouldIncludeTool("search for React tutorials", "fetch_url")).toBe(
      false,
    );

    expect(shouldIncludeTool("fetch https://example.com", "fetch_url")).toBe(
      true,
    );
  });
