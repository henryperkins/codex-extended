...
async function getSandbox(runInSandbox: boolean): Promise<SandboxType> {
  if (runInSandbox) {
    // Users may explicitly opt-out of any sandboxing by setting the
    // CODEX_UNSAFE_ALLOW_NO_SANDBOX environment variable (or via the
    // --dangerously-auto-approve-everything flag which sets this env var).
    // In that case we bypass all sandbox logic regardless of the host OS.
